...
class LRUCache { 
	private HashMap<Integer, Node4> map; 
	private int capacity, count; 
	private Node4 head, tail; 
	...
	public void deleteNode(Node4 node) 
	{ 
		node.pre.next = node.next; 
		node.next.pre = node.pre; 
	} 

	public void addToHead(Node4 node) 
	{ 
		node.next = head.next; 
		node.next.pre = node; 
		node.pre = head; 
	...
	{ 
		...
		if (map.get(key) != null) { 
			...
			Node4 node = new Node4(key, value); 
			map.put(key, node); 
			if (count < capacity) { 
				count++; 
				addToHead(node); 
